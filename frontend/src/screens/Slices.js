    ...
    <div>
      ...
      <br />
        ...
        <Row className='justify-content-center'>
          <h1 style={{ color: "white", fontFamily: "'Playfair Display', serif" }}>Slices</h1>
        </Row>

        {posts.length > 0 ? (
          posts.filter(post => post.isSlice).map((post, index) => (
            <React.Fragment key={index}>
              <Row className='justify-content-center'>
                <Post 

                    date={formatTimestamp(post.created_date)} user={post.user} caption={post.caption} description={post.description} total_likes={post.total_likes} total_bookmarks={post.total_bookmarks} 
                
                total_comments={successCreate ? post.total_comments + 1 : post.total_comments}
                
                avi={post.user_avi} name={post.user_name} id={post.id} 
                likers={post.likers}
          ...
          <div>
          <h6>Hey There It seems We Have No slices from anyone you follow yet</h6>
          <h6><Link style={{ color: "red" }} to='/galleria'>eXplore</Link></h6>
        </div>
        )}
